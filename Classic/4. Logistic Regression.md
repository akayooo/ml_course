...
${\hat y = \sigma\!\left(\beta_0 + \sum_{i} \beta_i x_i\right) = \dfrac{1}{1+e^{-\left(\beta_0 + \sum_{i} \beta_i x_i\right)}}}$

На простом языке:

- ${\beta_0}$ — смещение (bias), влияет на базовую вероятность
- ${\beta_i}$ — коэффициенты для каждого признака ${x_i}$
- Сумма ${\beta_0 + \sum_i \beta_i x_i}$ — это линейная часть
- Сигмоида преобразует линейный результат в вероятность

### Интерпретация коэффициентов

В логистической регрессии коэффициенты ${\beta_i}$ отвечают за логарифм отношения шансов (log-odds):

${\ln\!\left(\dfrac{p}{1-p}\right) = \beta_0 + \sum_{i} 
\beta_i x_i}$

Расшифровка:

- Если ${\beta_i>0}$ → при увеличении признака ${x_i}$ вероятность положительного класса растет
- Если ${\beta_i<0}$ → при увеличении признака ${x_i}$ вероятность положительного класса падает
- Величина ${|\beta_i|}$ показывает силу влияния

Пример: Если ${\beta_{age}=0.05}$, то каждый год жизни увеличивает log-odds на ${0.05}$ (примерно на 5% изменяется odds).
...
${J(\theta)= -\dfrac{1}{m}\sum_{i=1}^{m} \Big( y^{(i)} \log \hat y^{(i)} + (1-y^{(i)})\, \log (1-\hat y^{(i)}) \Big)}$
...
1. Инициализация: случайно выбираем начальные коэффициенты ${\beta_i}$
2. Forward pass: для каждого примера вычисляем ${\hat y=\sigma(\beta_0+\sum_i \beta_i x_i)}$
...
${\text{TPR}=\dfrac{TP}{TP+FN}}$ (Recall)

${\text{FPR}=\dfrac{FP}{FP+TN}}$ (${1-\text{Specificity}}$)
...
1. Данные: признаки ${x_i}$ и истинные метки ${y}$
2. Логистическая регрессия: обучается минимизировать log-loss, получает коэффициенты ${\beta_i}$
3. Вероятности: модель предсказывает ${\hat y\in[0,1]}$
4. Пороговое решение: выбираем порог (обычно ${0.5}$): если ${\hat y\ge 0.5}$ → класс 1, иначе 0
