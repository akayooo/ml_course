...
**Простыми словами:** 
- $\hat{y}$ — это наше предсказание (например, количество проданного мороженого)
- $x$ — признак (например, температура на улице)
- $\beta_0$ — базовый уровень (продажи при нулевой температуре)
- $\beta_1, \beta_2, ...$ — коэффициенты, которые модель учится подбирать

**Пример с температурой и мороженым:**
- $\beta_0 = 10$ (10 порций продаётся даже при 0°C)
- $\beta_1 = 2$ (на каждый градус тепла продажи растут на 2 порции)
- $\beta_2 = -0.05$ (но при высоких температурах рост замедляется)

Тогда формула: $\hat{y} = 10 + 2x - 0.05x^2$

где $n$ — степень полинома (максимальная степень $x$ в уравнении).
...
**О степени полинома:** 
- **1-я степень** (линейная регрессия): прямая линия. Подходит только для простых зависимостей.
  *Пример:* $y = 2x + 1$ — чем больше x, тем больше y.
  
- **2-я степень** (квадратичная): парабола. Моделирует один изгиб.
  *Пример:* $y = -x^2 + 4x + 1$ — рост с последующим спадом.
  
- **3-я степень** (кубическая): S-образная кривая. Два изгиба.
  *Пример:* $y = x^3 - 6x^2 + 4x + 10$ — сначала рост, затем спад, снова рост.

**Важно!** 
- Степень = количество изгибов + 1
- Слишком высокая степень (10+) ведёт к переобучению — модель «запоминает» шум в данных.
- На практике редко используют степень выше 3-4 без регуляризации.

**Наглядный пример:**
```
Степень 1:  y = 2x + 1         → / (прямая)
Степень 2:  y = -x² + 4x + 1   → Ʌ (парабола)
Степень 3:  y = x³ - 6x² + 4x + 10 → ~ (S-кривая)
```

> [!TIP]
> **Условие числа и численная стабильность**
> 
> **Проблема:** При высоких степенях полинома (например, $x^{10}$) возникают огромные числа. Например, если $x=10$, то $x^{10} = 10 000 000 000$. Матрица $X^TX$ становится «плохо обусловленной» — маленькие изменения в данных вызывают большие изменения в коэффициентах.
>
> **Пример плохой обусловленности:**
> ```
> X = [[1, 10, 100],    # x, x², x³ при x=10
>      [1, 20, 400]]   # x, x², x³ при x=20
> ```
> Столбцы становятся почти линейно зависимыми, и компьютер не может точно вычислить обратную матрицу.
>
> **Решение:**
> 1. **Масштабирование:** Привести все признаки к диапазону [-1, 1] или [0, 1]
> 2. **Центрирование:** Вычесть среднее значение
> 3. **Ортогональные полиномы:** Использовать полиномы Чебышёва или Лежандра, которые автоматически ортогональны
>
> **Практический совет:** Всегда используйте `StandardScaler` перед `PolynomialFeatures` в scikit-learn.
...
**Разберём по частям:**
1. $\sum (y_i - \hat{y}_i)^2$ — обычная MSE (среднеквадратичная ошибка)
2. $\lambda \sum \beta_j^2$ — штраф за большие веса (L2-регуляризация)

**Параметр $\lambda$ (лямбда):**
- $\lambda = 0$: выключает регуляризацию (чистый МНК)
- $\lambda \to \infty$: все коэффициенты стремятся к 0
- Оптимальное $\lambda$ находят перебором (например, с кросс-валидацией)

**Пример с данными о недвижимости:**
```
Без регуляризации:
Цена = 100000 + 50000*комнаты + 1000*площадь - 20000*возраст

С L2-регуляризацией (λ=0.1):
Цена = 95000 + 48000*комнаты + 980*площадь - 19500*возраст
```
Видно, что коэффициенты стали меньше, но не обнулились.
...
**Чем отличается от Ridge?**
- Штрафуется сумма модулей коэффициентов (L1-норма)
- Приводит к обнулению неважных признаков
- Выполняет автоматический отбор признаков

**Пример с данными о недвижимости:**
```
Без регуляризации:
Цена = 100000 + 50000*комнаты + 1000*площадь - 20000*возраст + 500*вид_на_парк

С L1-регуляризацией (λ=0.5):
Цена = 98000 + 49000*комнаты + 0*площадь - 19900*возраст + 0*вид_на_парк
```
Видно, что незначимые признаки (площадь, вид_на_парк) обнулились.
...
С ElasticNet (λ=0.5, α=0.7):
Цена = 98000 + 48000*комнаты + 50*площадь - 19500*возраст + 0*вид_на_парк
```
Видно, что:
- Незначимый признак (вид_на_парк) обнулился (как в LASSO)
- Остальные коэффициенты уменьшились (как в Ridge)
- Площадь почти обнулилась, но не полностью
